...
describe('extractKeyId', () => {
  ...
  describe('valid keys', () => {
    ...
    it('should extract keyId when secret contains trailing underscore', () => {
      const key = `VMX_${validKeyId}_secret_`;
      expect(extractKeyId(key)).toBe(validKeyId);
    });
  });
  ...
  describe('invalid structure', () => {
    ...
    it('should return keyId when there are more than 3 parts (extra after secret)', () => {
      expect(extractKeyId(`VMX_${validKeyId}_secret_extra`)).toBe(validKeyId);
    });

    it('should return keyId for a key with 4 underscore-separated parts', () => {
      expect(extractKeyId(`VMX_${validKeyId}_secret_extra_part`)).toBe(
        validKeyId,
      );
    });
