...
describe('digest', () => {
  ...
  beforeEach(() => {
    ...
    process.env['REDIS_KEY_SECRET'] = 'test-secret-key';
  });
  ...
  describe('REDIS_KEY_SECRET environment variable', () => {
    it('should throw when REDIS_KEY_SECRET is not set', () => {
      delete process.env['REDIS_KEY_SECRET'];
      expect(() => digest('test')).toThrow(
        'REDIS_KEY_SECRET is not configured',
      );
    });

    it('should throw when REDIS_KEY_SECRET is empty string', () => {
    ...
    it('should throw for both undefined and empty REDIS_KEY_SECRET', () => {
      process.env['REDIS_KEY_SECRET'] = '';
      expect(() => digest('test')).toThrow(
        'REDIS_KEY_SECRET is not configured',
      );
  ...
  describe('edge cases', () => {
    ...
    it('should produce a stable known digest value', () => {
      ...
      process.env['REDIS_KEY_SECRET'] = 'regression-secret';
      const result = digest('test');
      const expected = crypto
        .createHmac('sha256', 'regression-secret')
        .update('test')
        .digest('hex');
      expect(result).toBe(expected);
    });
